...
bot.on('message', message => {
    if(message.content[0] === PREFIX) {
        ...
        if(message.content === '!help') { 
            ...
            embed.addField('__', ':red_circle: Bonjour à tous, ce serveur est un serveur communautaire, merci de vous respecter les uns les autres et de ne pas être haineux ou irrespectueux sous peine d’une sanction. \n:red_circle: On vous connait ? Il suffit d’envoyer un petit message pour recevoir l’affectation que nous jugerons la plus raisonnable.\n:red_circle: Vous êtes nouveau ? N’hésitez pas a demander un accès aux channels résérvés  (GTA, LMS/PUBG, Rainbow Six Siege, et FiveM). N’hésitez, par ailleurs, pas à demander la création de grades et de salons réservés a de nouveaux jeux que vous jugez utiles.\n:heavy_minus_sign: Les grades administrateur et modérateur ne sont pas accessible au grand public, merci de ne pas demander d’acquérir ces grades.\n:red_circle: - Amusez vous bien sur ce serveur et n’hésitez pas à le partager a vos amis !');
            channel.send({embed});
            ...
            if(message.member.voiceChannel){
                message.member.voiceChannel.join().then(message.channel.send('Connecté au salon : \'' + message.member.voiceChannel.name +'\''));
                    ...
                    message.channel.send('Erreur, vous devez être connecté a un salon vocal.')
                }
...
bot.on('guildMemberAdd', member => {
    member.createDM().then(channel => {
        return channel.send('Bienvenue sur mon serveur ! \nTapez !règles pour reçevoir les règles du serveur par message privé, ' + member.displayName + ' !');
    }).catch(console.error)
});
